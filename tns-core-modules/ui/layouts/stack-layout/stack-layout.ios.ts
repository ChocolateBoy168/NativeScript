        ...
        this.eachLayoutChild((child, last) => {
            if (isVertical) {
                ...
                if (measureSpec === layout.AT_MOST && this.isUnsizedScrollableView(child)) {
                    trace.write("Avoid using vertically-scrollable views with no explicit height set inside StackLayout. Doing so might results in poor user interface performance and a poor user experience.", trace.categories.Error, trace.messageType.error);
                }
